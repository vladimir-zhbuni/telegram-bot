...
def extract_status_change(
    ...
    """
    logger.info(chat_member_update)
...
def greet_chat_members(update: Update, context: CallbackContext) -> None:
    ...
    name = update.chat_member.new_chat_member.user.name
    member_name = update.chat_member.new_chat_member.user.mention_html()
    if not was_member and is_member:
        logger.info(update.chat_member.new_chat_member.user.name)
        context.bot.send_message(chat_id=-1001483548883,
                                 text=f"Пользователь {member_name} {name} добавился на канал.",
                                 parse_mode=ParseMode.HTML,
                                 )
    elif was_member and not is_member:
        logger.info(update.chat_member)
        context.bot.send_message(chat_id=-1001483548883,
                                 text=f"Пользователь {member_name} {name} отписался от канала.",
                                 parse_mode=ParseMode.HTML,
                                 )
